...
struct IntelKernelTersoff : public lmp_intel::vector_routines<flt_t, acc_t, mic> {
  ...
  static void kernel_step_const_i(
    int eatom, int vflag, 
    int numneigh_i, int cnumneigh_i,
    const int * _noalias const firstneigh, int ntypes, 
    typename IntelBuffers<flt_t,acc_t>::atom_t * _noalias const x,
    const c_inner_t * _noalias const c_inner, 
    const c_outer_t * _noalias const c_outer, 
    ...
    *timer_compute = MIC_Wtime();
    ...
    #endif
    {
...

      {
        ...
        if (VL < 4) {
          IntelKernelTersoff<flt_t,acc_t,lmp_intel::NONE,false>::template kernel<EVFLAG,EFLAG>(ARGS);
        } else if (VL >= 8) {
          IntelKernelTersoff<flt_t,acc_t,lmp_intel::mode,true >::template kernel<EVFLAG,EFLAG>(ARGS);
        } else {
          IntelKernelTersoff<flt_t,acc_t,lmp_intel::mode,false>::template kernel<EVFLAG,EFLAG>(ARGS);
        }
	if (EVFLAG) {
                                          ...
                                          IntelBuffers<flt_t,acc_t> *buffers)
{
  ...
  #pragma offload_transfer target(mic:_cop) \
    in(c_outer : length(tp1sq) alloc_if(0) free_if(0)) \
...
void PairTersoffIntel::ForceConst<flt_t>::set_ntypes(const int ntypes,
  ...
  if ( (ntypes != _ntypes) ) {
    if (_ntypes > 0) {
      #ifdef _LMP_INTEL_OFFLOAD
      if (c_inner != NULL && c_outer != NULL &&  _cop >= 0) {
        c_inner_t * oc_inner = c_inner[0][0];
        c_outer_t * oc_outer = c_outer[0];
        #pragma offload_transfer target(mic:cop) \
          nocopy(oc_inner, oc_outer: alloc_if(0) free_if(1))
      }
      #endif
    ...
    if (ntypes > 0) {
      ...
      if (c_inner != NULL && c_outer != NULL && cop >= 0) {
        c_inner_t * oc_inner = c_inner[0][0];
        c_outer_t * oc_outer = c_outer[0];
        #pragma offload_transfer target(mic:cop) \
...
void IntelKernelTersoff<flt_t, acc_t, mic, pack_i>::kernel_step(
  ...
  ivec vcnumneigh_i = v::int_load_vl(cnumneigh);
  ivec vnumneigh_i = v::int_load_vl(numneigh);
  ...
  // 4. the loop that computes force contributions for the remainder
  {
    ...
    while (! v::mask_testz(vactive_mask) && cache_idx < N_CACHE) {
      ...
      vks = v::int_mullo(v_i4floats, v_i_NEIGHMASK &
          v::template int_gather<4>(vks, vactive_mask, vkks + vcnumneigh_i, firstneigh));
...
void IntelKernelTersoff<flt_t,acc_t,mic, pack_i>::kernel_step_const_i(
  ...
  if (EVFLAG && EFLAG && eatom) {
    f[i].w += v::acc_reduce_add(v::acc_mask_add(v::acc_zero(), vmask, vfwtmp, v::zero()));
  }
}

static const int N_PERATOM = 20;
template<class flt_t, class acc_t, lmp_intel::CalculationMode mic, bool pack_i>
template<bool EVFLAG, bool EFLAG>
void IntelKernelTersoff<flt_t,acc_t,mic, pack_i>::kernel(
  ...
  for (ii = iifrom; ii < iito; ii++) {
    ...
    int jnum_reduced = 0;
    int neighbuf_idx = (current_neighbuf + 1) % (v::VL + 1);
    int neighbuf_offs = neighbuf_idx * N_PERATOM;//(v::VL + 2);
    for (jj = 0; use_reduced && jj < jnum; jj++) {
      ...
      if (rsq < cutsqmax) {
        if (jnum_reduced >= N_PERATOM) {
          use_reduced = false;
          for (int i = 0; i < compress_idx; i++) {
            vnumneigh[i] = numneigh[is[i]];
            vcnumneigh[i] = cnumneigh[is[i]];
          }
          break;
        }
        neighbuf[neighbuf_offs + jnum_reduced++] = j;
      }
    }
    if (jnum_reduced == 0 && use_reduced) continue;
    current_neighbuf = neighbuf_idx;
    for (jj = 0; jj < jnum; jj++) {
      int j = firstneigh[jlist_off_i + jj] & NEIGHMASK;
      int w_j = x[j].w;
      flt_t dx_ij = x[j].x - x_i;
      ...
      if (rsq < cutsq) {
        is[compress_idx] = ii;
        js[compress_idx] = j;
        vnumneigh[compress_idx] = use_reduced ? jnum_reduced : jnum;
        vcnumneigh[compress_idx] = use_reduced ? neighbuf_offs : cnumneigh[i];
      ...
      if (compress_idx == v::VL) {
        ...
        if (pack_i) {
          kernel_step<EVFLAG,EFLAG>(
              eatom, vflag, 
              vnumneigh, vcnumneigh, use_reduced ? neighbuf : firstneigh, ntypes,
              x, c_inner, c_outer, f,
          ...
          kernel_step_const_i<EVFLAG,EFLAG>(
              eatom, vflag, 
              use_reduced ? vnumneigh[0] : jnum, 
              use_reduced ? vcnumneigh[0] : cnumneigh[i], 
              use_reduced ? neighbuf : firstneigh, ntypes,
              x, c_inner, c_outer, f,
              &vsevdwl, &vsv0, &vsv1, &vsv2, &vsv3, &vsv4, &vsv5, compress_idx, 
              ii, js, vmask_repulsive
          );
        }
        compress_idx = 0;
