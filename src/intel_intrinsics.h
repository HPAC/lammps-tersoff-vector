...
struct vector_ops<double, AVX> {
    ...
    static fvec gather(const fvec &from, const bvec &mask, const ivec &idx, const void *base) {
      ...
      //int imask = _mm256_movemask_pd(static_cast<F64vec4>(mask));
      //if (imask & (1 << 0)) result[0] = *reinterpret_cast<const double*>(reinterpret_cast<const char*>(base) + scale * idxs[2*0]);
      //if (imask & (1 << 1)) result[1] = *reinterpret_cast<const double*>(reinterpret_cast<const char*>(base) + scale * idxs[2*1]);
      //if (imask & (1 << 2)) result[2] = *reinterpret_cast<const double*>(reinterpret_cast<const char*>(base) + scale * idxs[2*2]);
      //if (imask & (1 << 3)) result[3] = *reinterpret_cast<const double*>(reinterpret_cast<const char*>(base) + scale * idxs[2*3]);
      for (int i = 0; i < VL; i++) {
...
struct vector_ops<float, AVX> {
    ...
    static fvec gather(const fvec &from, bvec mask, const ivec &idx, const void *base) {
      ...
      iarr idxs;
      _mm256_store_si256(reinterpret_cast<__m256i*>(idxs), idx);
      int imask = _mm256_movemask_ps(static_cast<F32vec8>(mask));
      if (imask & (1 << 0)) result[0] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[0]);
      if (imask & (1 << 1)) result[1] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[1]);
      if (imask & (1 << 2)) result[2] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[2]);
      if (imask & (1 << 3)) result[3] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[3]);
      if (imask & (1 << 4)) result[4] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[4]);
      if (imask & (1 << 5)) result[5] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[5]);
      if (imask & (1 << 6)) result[6] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[6]);
      if (imask & (1 << 7)) result[7] = *reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * idxs[7]);
      return blend(mask, from, _mm256_load_ps(reinterpret_cast<float*>(result)));
...
#pragma pack(push, 16)
  struct fvec {
    float32x4_t data;  
    fvec() {}
    explicit fvec(const float f) { data = vdupq_n_f32(f); }
    explicit fvec(const float f[4]) { data = vld1q_f32(f); }
    //fvec(const fvec &a) { data = a.data; }
    fvec(const float32x4_t &a) { data = a; }
    ...
    const fvec operator +(const fvec &b) const {
      return vaddq_f32(data, b.data);
    }
    const fvec operator -(const fvec &b) const {
      return vsubq_f32(data, b.data);
    }
    const fvec operator *(const fvec &b) const {
      return vmulq_f32(data, b.data);
    }
    const fvec operator /(const fvec &b) const {
      float32x4_t e = vrecpeq_f32(b.data);
      e = vmulq_f32(e, vrecpsq_f32(e, b.data));
      e = vmulq_f32(e, vrecpsq_f32(e, b.data));
      return vmulq_f32(data, e);
    }
    fvec& operator +=(const fvec &a) {
      data = vaddq_f32(data, a.data); return *this;
    }
    fvec& operator -=(const fvec &a) {
      data = vsubq_f32(data, a.data); return *this;
    }
    fvec& operator *=(const fvec &a) {
      data = vmulq_f32(data, a.data); return *this;
    }
    fvec& operator /=(const fvec &a) {
  ...
  fvec invsqrt(const fvec &a) {
    float32x4_t e = vrsqrteq_f32(a.data);
    e = vmulq_f32(e, vrsqrtsq_f32(vmulq_f32(e, a.data), e));
    e = vmulq_f32(e, vrsqrtsq_f32(vmulq_f32(e, a.data), e));
    return e;
  }
  ...
  fvec exp(const fvec &a) {
    return arm_neon::exp_ps(a.data);
    //float32x4_t k = round2even(a.data * invlog_2);
    //float32x4_t x = a.data - k * log_2hi;
    //x = x - k * log2_lo;
    //float32x4_t y = 5.0000e-1 * (x + 1.6666277e-1 * (x + 4.1665401e-2 * (x + 8.3955629e-3 * (x + 1.3997796e-3))));
    //float32x4_t x = 1 + (x + y * x*x);
    //int32x4_t ik = vcvtq_s32_f32(k);
    //ik += 127; // Emax
    //ik = vshlq_n_s32(ik, 23); // nmantissa
    //return x * vreinterpretq_f32_s32(ik);
  }
  fvec sin(const fvec &a) {
    return arm_neon::sin_ps(a.data);
  }
  fvec pow(const fvec &a, const fvec &b) {
    fvec c = arm_neon::log_ps(a.data);
    fvec d = c * b;
    return arm_neon::exp_ps(d);
  }
  struct ivec {
    int32x4_t data;
    ivec () {}
    explicit ivec(const int f) { data = vdupq_n_s32(f); }
    explicit ivec(const int f[4]) { data = vld1q_s32(f); }
    ivec(const int32x4_t &d) { data = d; }
    operator int32x4_t() const {
      return data;
    }
    const ivec operator &(const ivec &b) const {
      return vandq_s32(data, b);
    }
    const ivec operator |(const ivec &b) const {
      return vorrq_s32(data, b);
    }
    const ivec operator +(const ivec &b) const {
      return vaddq_s32(data, b);
    }
  };
  struct bvec {
    int32x4_t data;
    bvec() {}
    explicit bvec(int i) {
      if (i == 0) data = vdupq_n_s32(+0);
      else data = data = vdupq_n_s32(-1);
    }
    explicit bvec(const int32x4_t &d) { data = d; }
    explicit bvec(const uint32x4_t &d) { data = vreinterpretq_s32_u32(d); }
    friend bvec operator &(const bvec &a, const bvec &b) {
      return bvec(vandq_s32(a.data, b.data));
    }
    friend bvec operator |(const bvec &a, const bvec &b) {
      return bvec(vorrq_s32(a.data, b.data));
    }
    friend bvec operator ~(const bvec &a) {
      return bvec(vmvnq_s32(a.data));
    }
    bvec& operator &=(const bvec &a) {
      data = vandq_s32(data, a.data);
      return *this;
    }
  };
#pragma pack(pop)
...
template<>
struct vector_ops<float, NEON> {
    static const int VL = 4;
    static const int ALIGN = 16;
    typedef float fscal;
    typedef arm_neon::fvec fvec;
    typedef arm_neon::ivec ivec;
    typedef arm_neon::bvec bvec;
    typedef float farr[4];
    typedef int iarr[4];
    static fvec recip(const fvec &a) {
      float32x4_t e = vrecpeq_f32(a.data);
      e = vmulq_f32(e, vrecpsq_f32(e, a.data));
      e = vmulq_f32(e, vrecpsq_f32(e, a.data));
      return e;
    }
    template<int scale>
    ...
    static fvec gather(const fvec &from, const bvec &mask, const ivec &idx, const void *base) {
      fvec result = from;
      #define ARM_NEON_GATHER_UNROLL(i) \
        if (vgetq_lane_s32(mask.data, i) != 0.0)\
          result = vsetq_lane_f32(*reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + scale * vgetq_lane_s32(idx, i)), result, i);
      ARM_NEON_GATHER_UNROLL(0) 
      ARM_NEON_GATHER_UNROLL(1) 
      ARM_NEON_GATHER_UNROLL(2) 
      ARM_NEON_GATHER_UNROLL(3) 
      return result;
    }
    template<class T>
    static void gather_x(const ivec &idxs, const bvec &mask, const T *base, fvec *x, fvec *y, fvec *z, ivec *w) {
      float temp[16] __attribute__((aligned(16)));
      #define ARM_NEON_GATHER_X_UNROLL(i) \
        if (vgetq_lane_s32(mask.data, i) != 0)\
          vst1q_f32(&temp[4*i], vld1q_f32(reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + 1 * vgetq_lane_s32(idxs, i))));
      ARM_NEON_GATHER_X_UNROLL(0)
      ARM_NEON_GATHER_X_UNROLL(1)
      ARM_NEON_GATHER_X_UNROLL(2)
      ARM_NEON_GATHER_X_UNROLL(3)
      float32x4x4_t res = vld4q_f32(temp);
      *x = blend(mask, *x, res.val[0]);
      *y = blend(mask, *y, res.val[1]);
      *z = blend(mask, *z, res.val[2]);
      *w = int_blend(mask, *w, vreinterpretq_s32_f32(res.val[3]));
    }
    static void gather_8(const ivec &idxs, const bvec &mask, const void *base, 
        fvec *r0, fvec *r1, fvec *r2, fvec *r3, fvec *r4, fvec *r5, fvec *r6, fvec *r7) {
      fvec a = zero(), b = zero(), c = zero(), d = zero();
    ...
    static void gather_4(const ivec &idxs, const bvec &mask, const void *base, 
        fvec *r0, fvec *r1, fvec *r2, fvec *r3) {
      float temp[16] __attribute__((aligned(16)));
      #define ARM_NEON_GATHER_4_UNROLL(i) \
        if (vgetq_lane_s32(mask.data, i) != 0.0)\
          vst1q_f32(&temp[4*i], vld1q_f32(reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + 4 * vgetq_lane_s32(idxs, i))));
      ARM_NEON_GATHER_4_UNROLL(0)
      ARM_NEON_GATHER_4_UNROLL(1)
      ARM_NEON_GATHER_4_UNROLL(2)
      ARM_NEON_GATHER_4_UNROLL(3)
      float32x4x4_t res = vld4q_f32(temp);
      *r0 = blend(mask, *r0, res.val[0]);
      *r1 = blend(mask, *r1, res.val[1]);
      *r2 = blend(mask, *r2, res.val[2]);
      *r3 = blend(mask, *r3, res.val[3]);
    }
    static fvec blend(const bvec &mask, const fvec &a, const fvec &b) {
      return vreinterpretq_f32_s32(vorrq_s32(vandq_s32(mask.data, vreinterpretq_s32_f32(b)), vandq_s32(vreinterpretq_s32_f32(a), (~mask).data)));
    }
    static ivec int_blend(const bvec &mask, const ivec &a, const ivec &b) {
      return vorrq_s32(vandq_s32(mask.data, b), vandq_s32(a, (~mask).data));
    }
    static fvec fmadd(const fvec &a, const fvec &b, const fvec &c) {
      return a*b + c;
    }
    ...
    static bvec cmpeq(const fvec &a, const fvec &b) {
      return bvec(vceqq_f32(a, b));
    }
    static bvec cmpnle(const fvec &a, const fvec &b) {
      return bvec(vcgtq_f32(a, b));
    }
    static bvec cmple(const fvec &a, const fvec &b) {
      return bvec(vcleq_f32(a, b));
    }
    static bvec cmplt(const fvec &a, const fvec &b) {
      return bvec(vcltq_f32(a, b));
    }
    static bvec int_cmpneq(const ivec &a, const ivec &b) {
      return ~bvec(vreinterpretq_s32_u32(vceqq_s32(a.data, b.data)));
    }
    static bvec int_cmplt(const ivec &a, const ivec &b) {
      return bvec(vcltq_s32(a, b));
    }
    static fvec invsqrt(const fvec &a) {
      return arm_neon::invsqrt(a);
    }
    static fvec sincos(fvec *c, const fvec &a) {
      fvec ret;
      arm_neon::sincos_ps(a.data, &ret.data, &c->data);
      return ret;
    }
    static fscal reduce_add(const fvec &a) {
      float32x2_t b = vadd_f32(vget_high_f32(a), vget_low_f32(a));
    ...
    static ivec int_gather(const ivec &from, bvec mask, const ivec &idx, const void *base) {
      ivec result = from;
      #define ARM_NEON_INT_GATHER_UNROLL(i) \
        if (vgetq_lane_s32(mask.data, i) != 0.0) \
          result = vsetq_lane_s32(*reinterpret_cast<const int*>(reinterpret_cast<const char*>(base) + scale * vgetq_lane_s32(idx, i)), result, i);
      ARM_NEON_INT_GATHER_UNROLL(0) 
      ARM_NEON_INT_GATHER_UNROLL(1) 
      ARM_NEON_INT_GATHER_UNROLL(2) 
      ARM_NEON_INT_GATHER_UNROLL(3) 
      return result;
    }
    static fvec mask_add(const fvec &src, const bvec &mask, const fvec &a, const fvec &b) {
    ...
    static void store(void *at, const fvec &a) {
      vst1q_f32(reinterpret_cast<float*>(at), a);
    }
    static void int_store(int *at, const ivec &a) {
      vst1q_s32(at, a);
    }
    ...
    static bool mask_test_at(const bvec &mask, int at) {
      switch (at) {
      case 0: return vgetq_lane_s32(mask.data, 0) != 0;
      case 1: return vgetq_lane_s32(mask.data, 1) != 0;
      case 2: return vgetq_lane_s32(mask.data, 2) != 0;
      case 3: return vgetq_lane_s32(mask.data, 3) != 0;
      }
      return 0;
    }
    static bool mask_testz(const bvec &mask) {
      uint32x2_t tmp = vreinterpret_u32_s32(vorr_s32(vget_low_s32(mask.data), vget_high_s32(mask.data)));
      return vget_lane_u32(vpmax_u32(tmp, tmp), 0) == 0;
    }
    static bvec mask_enable_lower(int n) {
      ivec a(0);
      if (n > 0) a = vsetq_lane_s32(-1, a, 0);
      if (n > 1) a = vsetq_lane_s32(-1, a, 1);
      if (n > 2) a = vsetq_lane_s32(-1, a, 2);
      if (n > 3) a = vsetq_lane_s32(-1, a, 3);
      return bvec(a);
    }
    static ivec int_load_vl(const int *a) {
      return vld1q_s32(a);
...
#ifdef __ARM_NEON__
using lmp_intel::arm_neon::sqrt;
using lmp_intel::arm_neon::sin;
using lmp_intel::arm_neon::exp;
using lmp_intel::arm_neon::pow;
using lmp_intel::arm_neon::invsqrt;
#endif
